...
**重要**：请逐一读取屏幕上每个位置的数字，哪怕是小的、不清晰的数字也不要跳过。如果某个位置没有数字，写"无"。"""

    elif analysis_type == "posture":
        prompt = """这是一张体态评估测试设备的屏幕截图。请仔细识别屏幕上**所有的数字、百分号、指标名称和结果**，不要遗漏任何数据。
...
**重要**：请逐一读取屏幕上每个位置的数字，哪怕是小的、不清晰的数字也不要跳过。如果某个位置没有数字，写"无"。"""
        ...
        prompt = """这是一张体成分分析仪的屏幕截图。请仔细识别屏幕上**所有的数字、百分号、指标名称和结果**，不要遗漏任何数据。
